...
module.exports = function(deployer, network, accounts) {
    ...
    let res = contracts.setting.getCollateralRate(Web3Utils.fromAscii('ETH'));
    console.log("CollateralRate: ", Web3Utils.fromWei(res, 'ether'));
    console.log("contracts deployment finished\n\n")

    ///////////////////////////////////////////////////////////////////////////////////////
    console.log("-------- mint synths -------- ");
    receipt = contracts.synthx.mintFromCoin({value:Web3Utils.toWei('20', 'ether')});

    bal = contracts.dUSD.balanceOf(accounts[0]);
    console.log("dUSD balance:", Web3Utils.fromWei(bal, 'ether'));
    dTokenBal = contracts.synthxDToken.balanceOf(accounts[0]);
    console.log("dToken balance:", Web3Utils.fromWei(dTokenBal, 'ether'));
    col = contracts.stats.getTotalCollateral(accounts[0])
    console.log("totalDebt:", Web3Utils.fromWei(col.totalDebt, 'ether'));


    console.log("\n-------- burn synths -------- ");
    new Promise(r => setTimeout(r, 2000)); // sleep
    contracts.synthx.burn(Web3Utils.fromAscii('ETH'), Web3Utils.toWei('2000', 'ether'));
    ...
    res = contracts.stats.getRewards(accounts[0]);
    console.log("rewards: ", Web3Utils.fromWei(res, 'ether'))

    res = contracts.stats.getWithdrawable(accounts[0]);
    console.log("getWithdrawable:", Web3Utils.fromWei(res, 'ether'));

    console.log("-------- claim rewards -------- ");
    contracts.synthx.claimReward();
    bal = contracts.synthxToken.balanceOf(accounts[0]);
    console.log("synthx balance:", Web3Utils.fromWei(bal, 'ether'));

    console.log("-------- trade -------- ");
    ///////////////  trade
    // dUSD => dTSLA
    contracts.synthx.trade(Web3Utils.fromAscii('dUSD'), Web3Utils.toWei('10000', 'ether'),  Web3Utils.fromAscii('dTSLA'));
    bal = contracts.dTSLA.balanceOf(accounts[0]);
    console.log("dTSLA balance:", Web3Utils.fromWei(bal, 'ether'));

    // dTSLA => dAPPLE
    contracts.synthx.trade(Web3Utils.fromAscii('dTSLA'), Web3Utils.toWei('13', 'ether'),  Web3Utils.fromAscii('dAPPLE'));
    bal = dAPPLE.balanceOf(accounts[0]);
    console.log("dAPPLE balance:", Web3Utils.fromWei(bal, 'ether'));

    // get synth asset
    res = contracts.stats.getAssets(Web3Utils.fromAscii('Synth'), accounts[0]);
    console.log("synth assets:", res)
    // get stake asset
    res = contracts.stats.getAssets(Web3Utils.fromAscii('Stake'), accounts[0]);
    console.log("stake assets:", res)
    // get vaullts
    res = contracts.stats.getVaults(accounts[0]);
    console.log("getVaults:", res)

    // getTotalCollateral
    res = contracts.stats.getTotalCollateral(accounts[0]);
