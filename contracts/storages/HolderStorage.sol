...
contract HolderStorage is RewardsStorage, IHolderStorage {
    ...
    function setBalance(address account, uint256 period, uint256 amount, uint256 total) external onlyManager(managerName) {
        _setBalance(account, period, amount);
        _setBalance(address(0), period, total);
    }

    function _setBalance(address account, uint256 period, uint256 amount) private {
    ...
    function getBalance(address account, uint256 period) external view returns (uint256) {
