...
contract StakerStorage is RewardsStorage, IStakerStorage {
    ...
    function incrementStaked(bytes32 stake, address account, uint256 amount) external onlyManager(managerName) returns (uint256) {
    ...
    function decrementStaked(bytes32 stake, address account, uint256 amount, string calldata errorMessage) external onlyManager(managerName) returns (uint256) {
